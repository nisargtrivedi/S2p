...
public ArrayList<CalendarCollection>  date_collection_arr;
	...
	public StudentCalendarAdapter(Context context, GregorianCalendar monthCalendar, ArrayList<CalendarCollection> date_collection_arr) {
	...
	public void setEventView(View v,int pos,TextView txt){
		...
		for (int i = 0; i < len; i++) {
			...
			String status=cal_obj.event_message;
			int len1=day_string.size();
			if (len1>pos) {
			
			if (day_string.get(pos).equals(date) && status.equalsIgnoreCase("Present")) {
				v.setBackgroundColor(Color.parseColor("#343434"));
				v.setBackgroundResource(R.drawable.rounded_calender_item);
				
					txt.setTextColor(Color.WHITE);
			}
			else if (day_string.get(pos).equals(date) && status.equalsIgnoreCase("Undefined")) {
					v.setBackgroundColor(Color.parseColor("#123456"));
					v.setBackgroundResource(R.drawable.rounded_calender_item_two);
					txt.setTextColor(Color.WHITE);
				}
			else if (day_string.get(pos).equals(date) && status.equalsIgnoreCase("Absent")){
				v.setBackgroundColor(Color.RED);
				v.setBackgroundResource(R.drawable.rounded_calender_item_three);
				txt.setTextColor(Color.WHITE);
			}
			else if (day_string.get(pos).equals(date) && status.equalsIgnoreCase("holiday_leave")){
				v.setBackgroundColor(Color.parseColor("#123456"));
				v.setBackgroundResource(R.drawable.rounded_calender_item_two);
				txt.setTextColor(Color.WHITE);
			}
...
public void getPositionList(String date,final Activity act){
		...
		for (int i = 0; i < len; i++) {
			...
			if (date.equals(event_date)) {
				StudentAttendanceCalenderActivity studentAttendanceCalenderActivity= (StudentAttendanceCalenderActivity) act;
				studentAttendanceCalenderActivity.refreshCalendar();
