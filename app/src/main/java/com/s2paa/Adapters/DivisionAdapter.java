...
public class DivisionAdapter extends ArrayAdapter<Division> {
    ...
    public DivisionAdapter(Context context, List<Division> items) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final Division item=fItems.get(position);
    ...
    private class SchoolFilter extends Filter {
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            fItems = (List<Division>)results.values;
            if(results.count > 0)
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (filterText == null || filterText.length() == 0) {
                ...
                List<Division> filteredList = new ArrayList<Division>();
                List<Division> unFilteredList = new ArrayList<Division>();
                synchronized (this) {
                    unFilteredList.addAll(items);
                }
                for (Division li : unFilteredList) {
